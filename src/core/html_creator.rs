...
pub fn create_title(html: &mut html_builder::Node) -> AdhanResult<()> {
    ...
    let _ = head.link().attr("rel='stylesheet'").attr("href='current_month.css'");
...
pub fn create_table_body(table: &mut html_builder::Node, month: &[Day]) -> AdhanResult<()> {
    ...
    for day in month {
        ...
        writeln!(data_row.td().attr("class='tg-baqh'"), "{}", day.date).map_err(|x| AdhanError::Unknown(Box::new(x)))?;
        writeln!(data_row.td().attr("class='tg-baqh'"), "{}", day.prayers[0].time)
            .map_err(|x| AdhanError::Unknown(Box::new(x)))?;
        writeln!(data_row.td().attr("class='tg-baqh'"), "{}", day.prayers[1].time)
            .map_err(|x| AdhanError::Unknown(Box::new(x)))?;
        writeln!(data_row.td().attr("class='tg-baqh'"), "{}", day.prayers[2].time)
            .map_err(|x| AdhanError::Unknown(Box::new(x)))?;
        writeln!(data_row.td().attr("class='tg-baqh'"), "{}", day.prayers[3].time)
            .map_err(|x| AdhanError::Unknown(Box::new(x)))?;
        writeln!(data_row.td().attr("class='tg-baqh'"), "{}", day.prayers[4].time)
            .map_err(|x| AdhanError::Unknown(Box::new(x)))?;
    }
    Ok(())
}
...
pub fn create_table_header(table: &mut html_builder::Node) -> AdhanResult<()> {
    ...
    for elem in ["Date", "Fajr", "Dhuhr", "Asr", "Maghrib", "Isha"] {
        writeln!(header_row.th().attr("class='tg-baqh'"), "{}", elem).map_err(|x| AdhanError::Unknown(Box::new(x)))?;
    }
    Ok(())
}
...
.tg tg-baqh{text-align:center;vertical-align:top}
    ...
    write_file(get_user_filepath(), std::path::PathBuf::from("current_month.css"), css.to_owned())
}